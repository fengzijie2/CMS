...
const {uploadExcelStorage} = require("../../middlewares/uploadFlies")
...
router.post("/studentTable", uploadExcelStorage.single("file"), async (req, res) => {
    /* 
        批量导入学员的表格表头必须标准(必须按照模板, 因此需要给用户提供1个.xlsx文件模板)。

        当用户上传了表格文件之后：
            1. 校验数据是否规范
                数据格式规范：例如手机号必须是11位、年龄必须是数字...
                数据内容值是否符合系统里定好的数据。例如只有web/Python/java类别, 不允许在导入数据时新增类别
            2. 取出表格中的所有数据, 给数据组合上字段名(组合数据)
            3. 将规范好的用户数据存储到数据库里
            4. 给出批量导入数据的反馈
                1. 全部导入成功
                2. 失败 / 部分成功部分失败。 给出反馈, 明确告诉使用者哪里写的有问题
    */

    const { file, userInfo } = req // 上传的文件对象
    // 取第1张子表里的数据, 过滤掉长度为0的数据数组(成员)
    const arr = xlsx.parse(file.path)[0].data.filter(item => item.length)
    ...
    // 取出所有的学员数据。 取出从下标1位置起往后的所有成员
    let allStudentDataArr = arr.slice(1)
    let total = allStudentDataArr.length
    // 如果total为0, 说明表中不存在学员数据
    if (!total) return res.status(400).send({ code: 400, msg: "请不要上传空数据表" })

    // 格式化所有学员数据。组合成符合数据库字段要求的键值对形式的数据
    let formatStudentDataArr = allStudentDataArr.map((valueItem) => {
        let tempStudent = {}
