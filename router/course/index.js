...
const {uploadCoverImageStorage} = require("../../middlewares/uploadFlies")
...
// 处理修改课程基础信息
router.post("/reviseCourseInfo", async (req, res) => {
    const { _id, sellingPrice, isSale, description } = req.body
    courseDB
        .updateOne({ _id }, { sellingPrice, isSale, description })
        .then(() => {
            res.send({ code: 200, msg: "课程基础信息修改成功" })
        })
        .catch(() => {
            res.status(400).send({ code: 400, msg: "课程基础信息修改失败" })
        })
})
